...
<add name='TestProcess' read-only='false'>
    <entities>
        <add name='TestData'>
            ...
            using (var outer = new ConfigurationContainer(new HumanizeModule()).CreateScope(xml)) {
                using (var inner = new TestContainer(new HumanizeModule()).CreateScope(outer, new ConsoleLogger(LogLevel.Debug))) {
